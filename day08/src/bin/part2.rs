...
fn find_antinodes_of_pairs(
    ...
    while second.0 >= 0 && second.0 < height as i64 && second.1 >= 0 && second.1 < width as i64 {
        antinodes.insert((second.0 as usize, second.1 as usize));
        second.0 += diff.0;
        second.1 += diff.1;
    }

    while first.0 >= 0 && first.0 < height as i64 && first.1 >= 0 && first.1 < width as i64 {
